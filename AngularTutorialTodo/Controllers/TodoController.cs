...
namespace AngularTutorialTodo.Controllers
{
    public class TodoController : ApiController
    {
                                                        ...
                                                        int? limit = null, int offset = 0)
        {
            ...
            IQueryable<Todo> todos = string.IsNullOrEmpty(sort) ? list.OrderBy(o => o.Priority)
                : list.OrderBy(String.Format("it.{0} {1}", sort, desc ? "DESC" : "ASC"));

            if (!string.IsNullOrEmpty(q) && q != "undefined") todos = todos.Where(t => t.TodoItem.Contains(q));

            if (offset > 0) todos = todos.Skip(offset);
            if (limit.HasValue) todos = todos.Take(limit.Value);
            return todos;
        }
